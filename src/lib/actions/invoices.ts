...
export async function createInvoiceAction(data: {
  ...
  const phaseData = structure === 'SCHEDULED'
    ? (data.phases ?? []).map((p) => ({
        name: p.name,
        amount: p.amount,
  ...
  const invoiceStatus =
    data.status === 'DRAFT'
      ? 'DRAFT'
      : structure === 'SCHEDULED'
        ? getScheduledInvoiceStatus(
            phaseData.map((phase) => ({ status: 'UNPAID', dueDate: phase.dueDate })),
          )
...
export async function updateInvoiceStatusAction(
  ...
  const invoice = await prisma.invoice.findFirst({
    ...
    include: { phases: { select: { status: true, dueDate: true } } },
  })
  if (!invoice) throw new Error('Invoice not found')

  const nextStatus =
    invoice.paymentStructure === 'SCHEDULED' && status !== 'DRAFT' && status !== 'CANCELLED'
      ? getScheduledInvoiceStatus(invoice.phases)
      : status
...
export async function markPhasePaidAction(phaseId: string) {
  ...
  const newStatus = getScheduledInvoiceStatus(allPhases)
